...
一个原生 Android 触控屏检测 App，用来快速判断设备是否存在触控死区、断触、跳点、多点触控异常，以及屏幕边缘或角落触控不稳定的问题。
...
- Android 设备触控屏基础检测
- 现场排查屏幕是否存在死区、断触、跳点
- 给测试、售后、实施或开发人员提供一个直接可用的触控验证工具
- 作为业务 App、网页或 WebView 之外的独立对照测试工具
...
如果这个 App 里画线顺畅、坐标稳定，而其他应用或页面仍然表现异常，那么更应该优先排查应用、网页、WebView 或系统渲染链路，而不是屏幕本身。
...
3. 如果这里一切正常，再去测试其他 App、网页或 WebView
4. 如果其他界面异常、但这个 App 很顺，优先怀疑上层应用或渲染性能
